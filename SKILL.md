...
4. **配图**：
   - 用户提供了图，或本环境有图像生成工具 → 按图风配图；
   - 都没有 → 正文不插图、封面用自动生成的纯色图，交付时提醒用户可自行补图。
5. **提交**：运行 `python upload.py --md 标题.md --author 作者 --digest 摘要` 生成草稿。
6. **交付**：报告草稿 media_id，提醒用户去后台「草稿箱」手动发布（未认证无法 API 发布）。
